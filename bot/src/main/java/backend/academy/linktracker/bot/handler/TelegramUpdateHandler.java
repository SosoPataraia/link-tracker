...
public class TelegramUpdateHandler {
    ...
    public void handleUpdate(Update update) {
        ...
        if (text.startsWith("/")) {
            ...
            if (commandKey.equals("/skip")) {
                switch (session.getState()) {
                    ...
                    default -> botAdapter.sendMessage(chatId, "Нет активной операции для пропуска.");
                }
                return;
            }
            ...
            botAdapter.sendMessage(chatId, "Неизвестная команда: " + commandKey + "\nВведите /help для справки.");
            return;
        }
        ...
        switch (session.getState()) {
            ...
            default -> botAdapter.sendMessage(chatId, "Введите команду. Используйте /help для справки.");
        }
    }

    private void handleLinkInput(long chatId, String url, UserSession session) {
        if (!isValidUrl(url)) {
            botAdapter.sendMessage(
                    chatId,
    ...
    private void handleFiltersInput(long chatId, String input, UserSession session) {
        ...
        try {
            ...
            botAdapter.sendMessage(chatId, "✅ Ссылка добавлена в отслеживание:\n" + url + tagsInfo + filtersInfo);
        } catch (Exception e) {
            log.atWarn()
                    .addKeyValue("url", url)
                    .addKeyValue("chatId", chatId)
                    .addKeyValue("error", e.getMessage())
                    .log("link.register.failed");
            botAdapter.sendMessage(chatId, "❌ Не удалось добавить ссылку. Попробуйте позже.");
        }
    }
